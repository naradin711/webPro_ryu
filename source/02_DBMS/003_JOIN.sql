...
SELECT W.ENAME "이름", W.SAL "급여" , DNAME "부서명", LOC "근무지" , M.ENAME "직속상사"
FROM EMP W, EMP M, DEPT D
WHERE W.DEPTNO = D.DEPTNO AND W.MGR = M.EMPNO (+) ;
...
SELECT W.ENAME "이름", W.SAL "급여" , S.GRADE "등급", DNAME "부서명" , M.ENAME "직속상사"
FROM EMP W, EMP M, DEPT D, SALGRADE S
WHERE W.DEPTNO = D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL
AND W.MGR = M.EMPNO (+) ORDER BY DNAME ;

--8. 이름, 급여, 등급, 부서명, 연봉, 직속상사명. 연봉=(급여+comm)*12 단 comm이 null이면 0
SELECT W.ENAME "이름", W.SAL "급여" , S.GRADE "등급", DNAME "부서명" , M.ENAME "직속상사", (SAL+NVL(COMM,0))*12  "연봉"
FROM EMP W, EMP M, DEPT D, SALGRADE S
WHERE W.DEPTNO = D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL
AND W.MGR = M.EMPNO (+) ORDER BY DNAME ;
