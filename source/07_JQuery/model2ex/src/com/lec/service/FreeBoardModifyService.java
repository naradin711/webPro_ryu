...
public class FreeBoardModifyService implements Service {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
			...
			try {
				...
				String frdateStr = mRequest.getParameter("frdate");
				System.out.println(5);
				String fip = request.getRemoteAddr();
				System.out.println(6);
				FileBoardDao fDao = FileBoardDao.getInstance();
				FileBoardDto fileboard = new FileBoardDto(ftitle, fcontent, ffilename, fip, fid);
				int result = fDao.modifyFileBoard(fileboard); 
				if (result == FileBoardDao.SUCCESS) { // 중복멤 없음 회원가입 가능
					System.out.println(7);
					session.setAttribute("fileboard", fileboard );
					request.setAttribute("FileBoardModifyResult", result); 
				} else {
					request.setAttribute("FileBoardModifyError", result);
				} 
