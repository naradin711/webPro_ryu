...
<body>
	...
	<table>
		...
		<c:if test="${list.size() != 0 }">
			...
			<div class="paging">
				<a href="${conPath }/TopView.do?pageNum=1">◀◀</a>
					&nbsp; &nbsp; &nbsp;
				<c:if test="${startPage > BLOCKSIZE }" >
					<a href="${conPath }/TopView.do?pageNum=${startPage-1 }">◀</a>
				</c:if>
				<c:forEach var="i" begin="${startPage }" end="${endPage }">
					...
					<c:if test="${i != pageNum }">
						[<a href="${conPath }/TopView.do?pageNum=${i}"> ${i } </a>]
					</c:if>
				</c:forEach>
				<c:if test="${endPage < pageCnt }">
					<a href="${conPath }/TopView.do?pageNum=${endPage + 1}"> ▶ </a>
				</c:if>
				&nbsp; &nbsp; &nbsp;
				<a href="${conPath }/TopView.do?pageNum=${pageCnt}">▶▶</a>
				<br> 
			</div> 
		</div>
